...
import { inWarmup, pickWarmupMission } from './warmup';
import { todayKey } from '../utils/date';

/**
 * 今日のデイリーミッションを生成する。
 * - 導入期(ストリーク育成前)はウォームアップを1件だけ出す
 * - 卒業後は完了数が少ない領域を優先し、各領域から最も実施回数の少ないタスクを
...
export function ensureMissions(state: AppState, now: Date = new Date()): AppState {
  ...
  return { ...state, missions: { dateKey: today, taskIds: pickMissions(state, now) } };
}

function pickMissions(state: AppState, now: Date): string[] {
  if (inWarmup(state, now)) return pickWarmupMission(state);

  // 卒業後はウォームアップを除いた本格タスクから選ぶ
  const active = state.tasks.filter((t) => !t.archived && !t.warmup);
